...
internal sealed class AppointmentService(
    ...
    IMapper mapper,
    IUserService userService) : IAppointmentService
{
    ...
    public async Task<Result<string>> CreateAsync(CreateAppointmentDto request, CancellationToken cancellationToken)
    {
        ...
        if (doctor == null || doctor.UserType is not UserType.Doctor)
        {
        ...
        {
            return Result<string>.Failure("Doctor not working that day");
        }

        IQueryable<Appointment> appointments = appointmentRepository.GetWhere(p=>p.DoctorId == request.DoctorId);
        ...
        return Result<string>.Succeed("Crete appointement is succeeded");
